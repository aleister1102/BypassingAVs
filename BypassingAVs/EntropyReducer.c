...
PLINKED_LIST InsertAtTheEnd(IN OUT PLINKED_LIST LinkedList, IN PBYTE pBuffer, IN INT ID)
{
	...
	if (!pNewNode)
		return NULL;
	CopyMemoryEx(pNewNode->pBuffer, pBuffer, BUFF_SIZE);
	pNewNode->ID = ID;
	pNewNode->Next = NULL;
...
BOOL Deobfuscate(IN PBYTE pFuscatedBuff, IN SIZE_T sFuscatedSize, OUT PBYTE* ptPayload, OUT PSIZE_T psSize)
{
	...
	while (pTmpHead != NULL) {
		...
		CopyMemoryEx(TmpBuffer, pTmpHead->pBuffer, BUFF_SIZE);
		...
		if (BufferBytes != NULL) {
			...
			CopyMemoryEx((PVOID)(BufferBytes + (BufferSize - BUFF_SIZE)), TmpBuffer, BUFF_SIZE);
		}
	...
	*psSize = x * BUFF_SIZE; // payload size
	...
	PLINKED_LIST pTmpHead2 = pTmpHead ? pTmpHead->Next : NULL;
